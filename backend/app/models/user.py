...
class User(BaseModel):
    ...
    def to_dict(self, with_roles=True, with_info=True, with_payments=True, with_routine=True):
        data = {
          'id':self.id,
          'name':self.name,
          'email':self.email,
        ...
        if with_routine:
          data['routine']= [routine.to_dict(with_user=False) for routine in self.routines]
        return data

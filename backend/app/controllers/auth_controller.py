...
class AuthController:
    ...
    def Register(request:dict) -> tuple[Response, int]:
        name:str | None = request.get('name')
        ...
        if error is None:
            try:
                if rol_user and name and password and email is not None:
                    user = User(name=name, email=email, rol_id=rol_user, password=password)    
